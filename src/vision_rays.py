...
def north_ray_empty(self, square_notation:str):
    ...
    for i in range(0,8):
        if square_index + i*8 + 8> 64:
            break
        vision_board = int(vision_board | 1 << int(square_index+i*8+8))
...
def west_ray_empty(self,square_notation:str):
    ...
    vision_board = int(0)
    for i in range(col):
        vision_board = int(vision_board | 1 << int(square_index-i-1))
    return int(vision_board), "WN", square_index
...
def south_west_ray_empty(self,square_notation:str):
    ...
    for i in range(min(row,col)):
        vision_board = int(vision_board | 1 << np.uint(square_index -9 * (i+1)))
    return int(vision_board), "WN", square_index
...
def south_east_ray_empty(self,square_notation:str):
    ...
    for i in range(min(row,7-col)):
        vision_board = int(vision_board | 1 << np.uint(square_index - 7 * (i+1)))
    return int(vision_board), "WN", square_index
    ...
    for move in king_moves:
        ...
        if  0 <= target <= 63:
            ...
            if abs(target_col - col) <= 1:
                king_vision_board_empty = int(king_vision_board_empty | 1 << int(square_index + move))
...
def black_pawn_vision_board_empty(self,square_notation): # EI TOIMI TEE SAMA KU WHITE PAWN FIX ASAP
    square_index = alg_notation_to_index[square_notation]
    pawn_moves = [-8]
    pawn_vision_board_empty = int(0)
    two_move_vision_ray = int(0)

    if int(square_notation[1]) == 1:
        #todo promote queen
        return int(0), int(0) #placeholder 
    if int(square_notation[1]) == 7:
        two_move_vision_ray = int(two_move_vision_ray | 1 << int(square_index-16))
    for move in pawn_moves:
        pawn_vision_board_empty = int(pawn_vision_board_empty | 1 << int(square_index+move))
    return pawn_vision_board_empty, two_move_vision_ray

def black_pawn_attack(self, square_notation):
    square_index = alg_notation_to_index[square_notation]
    pawn_attack_board_empty = int(0)
    if int(square_notation[1]) > 1:
        if square_notation[0] != "h":
            pawn_attack_board_empty = int(pawn_attack_board_empty | 1 << int(square_index-7))
        if square_notation[0] != "a":
            pawn_attack_board_empty = int(pawn_attack_board_empty | 1 << int(square_index-9))
